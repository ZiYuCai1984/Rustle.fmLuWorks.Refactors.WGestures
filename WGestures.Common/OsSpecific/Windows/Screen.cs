...
namespace WGestures.Common.OsSpecific.Windows
{
    public struct Screen
    {
        ...
        private static readonly object
            syncLock = new object(); //used to lock this class before [....]'ing to SystemEvents

        private static int desktopChangedCount = -1; //static counter of desktop size changes

        private int
            currentDesktopChangedCount; // = -1;//instance-based counter used to invalidate WorkingArea
        ...
        private static readonly bool multiMonitorSupport = User32.GetSystemMetrics(80) != 0;
        ...
        internal Screen(IntPtr monitor, IntPtr hdc)
        {
            ...
            if (!multiMonitorSupport || monitor == (IntPtr) PRIMARY_MONITOR)
            {
                ...
                {
                    var size = new Size(User32.GetSystemMetrics(0), User32.GetSystemMetrics(1));
            ...
            else
            {
                ...
                this.Bounds = Rectangle.FromLTRB(
                    ...
                    info.Monitor.Right,
                    info.Monitor.Bottom);
                this.Primary = (info.Flags & MONITORINFOF_PRIMARY) != 0;
                ...
                if (hdc == IntPtr.Zero)
                {
                    screenDC = GDI32.CreateDC(this.DeviceName, null, null, new HandleRef());
                }
            }

            hmonitor = monitor;

            this.BitsPerPixel = GDI32.GetDeviceCaps(new HandleRef(null, screenDC).Handle, 12);
            this.BitsPerPixel *= GDI32.GetDeviceCaps(new HandleRef(null, screenDC).Handle, 14);
        ...
        public Rectangle WorkingArea
        {
            get
            {
                ...
                if (currentDesktopChangedCount != DesktopChangedCount)
                {
                    Interlocked.Exchange(ref currentDesktopChangedCount, DesktopChangedCount);
        ...
        internal static Screen FromHandleInternal(IntPtr hwnd)
        {
            ...
            {
                return new Screen(
                    User32.MonitorFromWindow(new HandleRef(null, hwnd), MONITOR_DEFAULTTONEAREST));
            }
