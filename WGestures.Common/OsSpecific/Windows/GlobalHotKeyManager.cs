...
namespace WGestures.Common.OsSpecific.Windows
{
    ...
    public sealed class GlobalHotKeyManager : IDisposable
    {
        ...
        private readonly Dictionary<string, HotKey> _idToHotKey = new Dictionary<string, HotKey>();

        private readonly Dictionary<HotKey, Action<HotKeyEventArgs>> _keyToAction =
            new Dictionary<HotKey, Action<HotKeyEventArgs>>();
        ...
        public GlobalHotKeyManager()
        {
            ...
            {
                if (this.HotKeyPreview != null)
                {
                    if (this.HotKeyPreview(this, this.GetIdOfHotKey(args.HotKey), args.HotKey))
                    {
                        return;
                    }
                }
        ...
        public bool RegisterHotKey(
            string id, ModifierKeys modifier, Keys key, Action<HotKeyEventArgs> callback)
        {
            var combined = new HotKey
                {modifiers = modifier, key = key};

            return this.RegisterHotKey(id, combined, callback);
        }

        public bool RegisterHotKey(string id, HotKey hk, Action<HotKeyEventArgs> callback)
        {
            Debug.WriteLine("RegisterHotKey: " + id + " " + hk);
            if (_idToHotKey.ContainsKey(id))
            {
                this.UnRegisterHotKeyById_internal(id);
            }
                ...
                (int) hk.key))
            {
                ...
                if (this.HotKeyRegistered != null)
                {
                    this.HotKeyRegistered(id, hk);
                }
        ...
        private void UnRegisterHotKeyById_internal(string id, bool publishEvent = false)
        {
            try
            {
                ...
                if (this.HotKeyUnRegistered != null)
                {
                    this.HotKeyUnRegistered(id, hk);
                }
            }
        ...
        public void UnRegisterHotKey(string id)
        {
            ...
            this.UnRegisterHotKeyById_internal(id, true);
        }
        ...
        public struct HotKey : IEquatable<HotKey>
        {
            ...
            public override bool Equals(object obj)
            {
                ...
                return this.Equals(asThis.Value);
            }
            ...
            public override string ToString()
            {
                ...
                if (0 != (modifiers & ModifierKeys.Control))
                {
                    sb.Append("Ctrl-");
                }

                if (0 != (modifiers & ModifierKeys.Shift))
                {
                    sb.Append("Shift-");
                }

                if (0 != (modifiers & ModifierKeys.Alt))
                {
                    sb.Append("Alt-");
                }

                if (0 != (modifiers & ModifierKeys.Win))
                {
                    sb.Append("Win-");
                }
        ...
        public class HotKeyEventArgs : EventArgs
        {
            ...
            public HotKeyEventArgs(HotKey hk)
            {
                HotKey = hk;
            }
        }
        ...
        private class Window : NativeWindow, IDisposable
        {
            ...
            protected override void WndProc(ref Message m)
            {
                ...
                if (m.Msg == WM_HOTKEY)
                {
                    ...
                    {
                        this.KeyPressed(this, new HotKeyEventArgs(new HotKey(modifier, key)));
                    }
                }
                else
                {
