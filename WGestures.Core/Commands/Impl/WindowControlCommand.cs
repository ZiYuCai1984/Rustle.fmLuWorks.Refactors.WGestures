...
namespace WGestures.Core.Commands.Impl
{
    ...
    public class WindowControlCommand : AbstractCommand, IGestureContextAware
    {
        ...
        {
            var cursorWin = Native.WindowFromPoint(
                new Native.POINT
                    {x = this.Context.StartPoint.X, y = this.Context.StartPoint.Y});

            this.DoOperation(cursorWin);
        }
        ...
        private void DoOperation(IntPtr win)
        {
            while (true)
            {
                ...
                switch (this.ChangeWindowStateTo)
                {
                    case WindowOperation.MAXIMIZE_RESTORE:
                        IntPtr winToControl;
                        if ((long) User32.WS.WS_MAXIMIZEBOX
                            == (topLevelWinstyle & (long) User32.WS.WS_MAXIMIZEBOX))
                        {
                            winToControl = topLevelWin;
                        }
                        else if (topLevelWin != rootWin
                                 && (long) User32.WS.WS_MAXIMIZEBOX
                                 == (rootWinStyle & (long) User32.WS.WS_MAXIMIZEBOX))
                        {
                        ...
                        else
                        {
                            User32.ShowWindowAsync(
                                winToControl,
                                (int) ShowWindowCommands.MAXIMIZED);
                        }

                        goto end;

                    case WindowOperation.MINIMIZE:
                        if ((long) User32.WS.WS_MINIMIZEBOX
                            == (rootWinStyle & (long) User32.WS.WS_MINIMIZEBOX))
                        {
                            User32.PostMessage(
                                rootWin,
                                User32.WM.WM_SYSCOMMAND,
                                (int) User32.SysCommands.SC_MINIMIZE,
                                0);
                        }
                        else if (topLevelWin != rootWin
                                 && (long) User32.WS.WS_MINIMIZEBOX
                                 == (topLevelWinstyle & (long) User32.WS.WS_MINIMIZEBOX))
                        {
                            User32.PostMessage(
                                topLevelWin,
                                User32.WM.WM_SYSCOMMAND,
                    ...
                    case WindowOperation.CLOSE:
                        User32.PostMessage(
                            rootWin,
                            User32.WM.WM_SYSCOMMAND,
                            (int) User32.SysCommands.SC_CLOSE,
                            0);
                        goto end;
