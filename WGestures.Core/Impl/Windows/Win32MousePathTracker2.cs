...
namespace WGestures.Core.Impl.Windows
{
    public class Win32MousePathTracker2 : IPathTracker
    {
        ...
        public Win32MousePathTracker2()
        {
            ...
            //properties defaults
            this.TriggerButton = /*GestureButtons.RightButton |*/ GestureTriggerButton.Middle;
            this.InitialValidMove = (int) (5 * dpiFactor);
            this.InitialStayTimeout = true;
            this.InitialStayTimeoutMillis = 150;

            this.EffectiveMove = (int) (10 * dpiFactor) * 2; //todo: 增加灵敏度调整
            this.StepSize =
                3; // EffectiveMove/4;// (int) (EffectiveMove * 0.8 * dpiFactor);// EffectiveMove/8;
            this.StayTimeout = false;
            this.PerformNormalWhenTimeout = false;

            _mouseKbdHook = new MouseKeyboardHook();
            _mouseKbdHook.MouseHookEvent += this.MouseHookProc;
            ...
            _edgeDetector.Rub += this.EdgeDetector_Rub;
        ...
        public bool StayTimeout
        {
            ...
            set
            {
                ...
                if (value && _stayTimer == null)
                {
                    ...
                    _stayTimer.Elapsed += this.StayTimeoutProc;
                }
        ...
        {
            get => _stayTimeoutMillis;
            set
            {
                _stayTimeoutMillis = value;
                if (_stayTimer != null)
                {
                    _stayTimer.Interval = value;
                }
            }
        }
        ...
        public int StepSize
        {
            ...
            set
            {
                if (value > this.EffectiveMove)
                {
                    throw new ArgumentException("SetpSize 不能大于 EffectiveMove");
                }
        ...
        public bool EnableWindowsKeyGesturing
        {
            ...
            set
            {
                if (value)
                {
                    ...
                    {
                        _mouseKbdHook.KeyboardHookEvent += this.KeyboardHookProc;
                    }
                }
                else
                {
        ...
        private readonly Queue<MSG> _msgQueue = new Queue<MSG>(16);
        ...
        public void Start()
        {
            ...
            while (true)
            {
                ...
                lock (_msgQueue)
                {
                    ...
                    this.UpdateContextAndEventArgs();
                }
                ...
                switch (msg.message)
                {
                    ...
                    case WM.GESTBTN_MOVE:
                        ...
                        break;
                    case WM.HOT_CORNER:
                        this.OnHotCorner((ScreenCorner) msg.param);
                        break;
                    case WM.RUB_EDGE:
                        this.OnRubEdge((ScreenEdge) msg.param);
                        break;
                    case WM.GESTBTN_MODIFIER:
                        this.OnModifier((GestureModifier) msg.param);
                        break;
                    case WM.GESTBTN_UP:
                        this.OnMouseUp(msg.param != 0);
                        break;
                    ...
                    case WM.GUI_REQUEST:
                        ...
                        {
                            if (this.RequestPauseResume != null)
                            {
                                this.RequestPauseResume(_isPaused); //todo: 必要这个参数吗
                            }
                        }
                        else if (msg.param == (int) GUI_RequestType.ShowHideTray)
                        {
                            if (this.RequestShowHideTray != null)
                            {
                                this.RequestShowHideTray();
                            }
                        }
        ...
        public bool Paused
        {
            ...
            set
            {
                if (_isStopped)
                {
                    throw new InvalidOperationException("已处于停止状态");
                }

                _isPaused = value;
                this.Post(WM.PAUSE_RESUME, value ? 1 : 0);
            }
        }
        ...
        {
            //处理 左键 + 中键 用于 暂停继续的情形
            if (this.HandleSpecialButtonCombination(e))
            {
                return;
            }
            ...
            var mouseData = (Native.MSLLHOOKSTRUCT) Marshal.PtrToStructure(
                e.lParam,
            ...
            if (_simulatingInput || mouseData.dwExtraInfo.ToInt64() == SIMULATED_EVENT_TAG)
            {
                ...
                if (this.InitialStayTimeout && _isInitialTimeout)
                {
            ...
            switch (m)
            {
                ...
                case MouseMsg.WM_XBUTTONDOWN:
                    if (!_captured)
                    {
                        if (m == MouseMsg.WM_MBUTTONDOWN
                            && (this.TriggerButton & GestureTriggerButton.Middle) == 0
                            || m == MouseMsg.WM_RBUTTONDOWN
                            && (this.TriggerButton & GestureTriggerButton.Right) == 0
                            || m == MouseMsg.WM_XBUTTONDOWN
                            && (this.TriggerButton & GestureTriggerButton.X) == 0)
                        {
                            return;
                        }
                        ...
                        try
                        {
                            ...
                            _captured = this.OnBeforePathStart();
                        }
                        catch (Exception ex)
                        {
#if DEBUG
                        ...
                        if (_captured)
                        {
                            ...
                            switch (m) //TODO: extract function
                            {
                                ...
                                case MouseMsg.WM_MBUTTONDOWN:
                                    ...
                                    break;
                                case MouseMsg.WM_XBUTTONDOWN:
                                    var x = (XButtonNumber) (mouseData.mouseData
                                                             >> 16); //which X Button
                                    _gestureBtn = x == XButtonNumber.One
                            ...
                            e.Handled = true;
                            this.Post(WM.GESTBTN_DOWN);
                        }
                    }
                    else //另一个键作为手势键的时候，作为修饰键
                    {
                        GestureModifier
                            gestMod; // = m == MouseMsg.WM_RBUTTONDOWN ? GestureModifier.RightButtonDown : GestureModifier.MiddleButtonDown;
                        ...
                        e.Handled = this.HandleModifier(gestMod);
                    }
                ...
                case MouseMsg.WM_MOUSEMOVE:
                    ...
                    else
                    {
                        ...
                        {
                            //忽略禁用列表
                            this.OnBeforePathStart();
                            _captured = true;
                            _gestureBtn = GestureTriggerButton.Right;
                            this.Post(WM.GESTBTN_DOWN, 1);
                        }

                        //未捕获的情况下才允许hotcorner
                        this.HotCornerHitTest();
                    }
                ...
                case MouseMsg.WM_MOUSEWHEEL:
                    ...
                    else if (DateTime.UtcNow - _modifierEventHappendPrevTime
                             < TimeSpan.FromMilliseconds(300)
                    ) //延迟一下，因为 中键手势 + 滚动，可能导致快捷键还没结束，而滚轮事件发送到了目标窗口，可鞥解释成其他功能（比如ctrl + 滚轮 = 缩放）
                    {
                        e.Handled = true;
                    }
                ...
                case MouseMsg.WM_LBUTTONDOWN:
                    if (_captured)
                    {
                        e.Handled = this.HandleModifier(GestureModifier.LeftButtonDown);
                    }
            ...
            if (e.key == Keys.LWin)
            {
                ...
                else
                {
                    if (_isVirtualGesturing) // && _isVirtualGesturing)
                    {
                        ...
                        {
                            _captured = false;
                            this.Post(WM.GESTBTN_UP, 1); //isVirtual
                        }
                        else
                        {
                            new Thread(
                                () =>
                                {
                                    ...
                                    sim.Keyboard.ModifiedKeyStroke(
                                        new[] {VirtualKeyCode.LWIN},
                                        new[] {(VirtualKeyCode) e.key});
        ...
        private void HotCornerHitTest()
        {
            ...
            var scr = Screen.ScreenBoundsFromPoint(_curPos);
            if (scr == null)
            {
                return;
            }
            ...
            {
                var p = this.GetBoundsCornerPoint(scr.Value, (ScreenCorner) corner);
                var dist = GetPointDistance(ref p, ref _curPos);

                if (!_isHotCornerReset
                ...
                else if (dist <= TRIGGER_DIST && _isHotCornerReset)
                {
                    ...
                    _lastTriggeredCorner = (ScreenCorner) corner;
                    this.Post(WM.HOT_CORNER, corner);
                }
            }
        }
        ...
        {
            this.Post(WM.RUB_EDGE, (int) edge);
        }
        ...
        private User32.MOUSEEVENTF MakeGestureBtnEvent(
            GestureTriggerButton btn, bool isUp, out uint data)
        {
            data = 0;
            switch (btn)
            {
                ...
                case GestureTriggerButton.X1:
                    data = 1;
                    return isUp
                        ? User32.MOUSEEVENTF.MOUSEEVENTF_XUP
                        : User32.MOUSEEVENTF.MOUSEEVENTF_XDOWN;
                case GestureTriggerButton.X2:
                    data = 2;
                    return isUp
                        ? User32.MOUSEEVENTF.MOUSEEVENTF_XUP
        ...
        {
            if (this.IsModifierFiltered(modifier))
            {
                return false;
            }
            ...
            if (modifier == (modifier & GestureModifier.Scroll))
            {
                ...
                {
                    this.Post(WM.GESTBTN_MODIFIER, (int) modifier);
        ...
        private bool HandleSpecialButtonCombination(MouseKeyboardHook.MouseHookEventArgs e)
        {
            ...
            var lButtonPressed =
                Native.GetAsyncKeyState(mouseSwapped ? Keys.RButton : Keys.LButton) < 0;
            ...
            if (e.Msg == MouseMsg.WM_MBUTTONDOWN && lButtonPressed)
            {
                if (shiftPressed)
                {
                    this.Post(WM.GUI_REQUEST, (int) GUI_RequestType.ShowHideTray);
                }
                else
                {
                    this.Post(WM.GUI_REQUEST);
                }
        ...
        private void UpdateContextAndEventArgs()
        {
            if (_moveCount == 0)
            {
                ...
                if (this.PreferWindowUnderCursorAsTarget)
                {
                    var fgWin = Native.WindowFromPoint(
                        new Native.POINT
                            {x = _curPos.X, y = _curPos.Y});
        ...
        {
            if (this.DisableInFullscreen && this.IsInFullScreenMode())
            {
                return false;
            }
            ...
            if (this.BeforePathStart != null)
            {
                this.BeforePathStart(args);
            }
        ...
        private void OnMouseDown()
        {
            //hack: dunno how
            var dpiFactor = Native.GetScreenDpi() / 96.0f;
            this.EffectiveMove = (int) (Screen.ScreenBoundsFromPoint(_curPos).Value.Width * 0.025f);
        ...
        {
            if (this.StayTimeout && _isTimeout)
            {
                return;
            }
            ...
            if (!_initialMoveValid)
            {
                ...
                if (this.InitialStayTimeout && !_isInitialTimeout)
                {
                    ...
                    if (now - _mouseDownTime
                        > TimeSpan.FromMilliseconds(this.InitialStayTimeoutMillis))
                    {
                ...
                if (initialMoveDist > this.InitialValidMove)
                {
                    ...
                    if (this.InitialStayTimeout)
                    {
                        ...
                        if (_isInitialTimeout)
                        {
                            ...
                            this.SimulateMouseEvent(
                                this.MakeGestureBtnEvent(_gestureBtn, false, out data),
                                _startPoint.X,
                                _startPoint.Y,
                                data);
                            return;
                    ...
                    if (this.PathStart != null)
                    {
                        ...
                        //Console.WriteLine('>');
                        this.PathStart(_currentEventArgs);
                    }
                }
                else
                {
            ...
            if (dist >= this.StepSize && this.PathGrow != null)
            {
                this.PathGrow(_currentEventArgs);
                _lastPoint = _curPos;
            }

            if (GetPointDistance(ref _curPos, ref _lastEffectivePos) > this.EffectiveMove
                && this.EffectivePathGrow != null)
            {
                this.EffectivePathGrow(_currentEventArgs);
                _lastEffectivePos = _curPos;
            }
        ...
        private void OnHotCorner(ScreenCorner corner)
        {
            ...
            if (!mousePressed && this.HotCornerTriggered != null)
            {
                this.HotCornerTriggered(corner);
            }
        }
        ...
        private void OnRubEdge(ScreenEdge edge)
        {
            ...
            var mousePressed = Native.GetAsyncKeyState(Keys.LButton) < 0
                               || Native.GetAsyncKeyState(Keys.RButton) < 0
                               || Native.GetAsyncKeyState(Keys.MButton) < 0;
            if (!mousePressed && this.EdgeRubbed != null)
            {
                this.EdgeRubbed(edge);
            }
        }
        ...
        private void OnModifier(GestureModifier modifier)
        {
            ...
            if (!_initialMoveValid && this.PathStart != null)
            {
                _initialMoveValid = true;
                _currentEventArgs.Location = _startPoint;
                this.PathStart(_currentEventArgs);
            ...
            {
                this.PathModifier(_currentEventArgs);
            }

            //如果已经被冻结，则不应继续计时
            if (!this.IsSuspended && _stayTimeout)
            {
                _stayTimer.Stop();
                _stayTimer.Start();
            }
        ...
        private void OnMouseUp(bool isVirtual = false)
        {
            ...
            if (!_initialMoveValid)
            {
                ...
                else
                {
                    ...
                    this.SimulateMouseEvent(
                        ...
                        data);
                    this.SimulateMouseEvent(
                        this.MakeGestureBtnEvent(_gestureBtn, true, out data),
                        _curPos.X,
                        _curPos.Y,
                        data);
                }
            ...
            if (this.PathEnd != null && _initialMoveValid && !_isTimeout)
            {
                this.PathEnd(_currentEventArgs);
            }
        ...
        private void OnTimeout()
        {
            ...
            if (this.PerformNormalWhenTimeout)
            {
            ...
            if (this.PathTimeout != null)
            {
                this.PathTimeout(_currentEventArgs);
            }
        }
        ...
        {
            var fgWindow = Native.GetAncestor(
                Native.GetForegroundWindow(),
                Native.GetAncestorFlags.GetRoot);
            var deskWindow = User32.GetDesktopWindow();
            var shellWindow = User32.GetShellWindow();


            if (fgWindow == IntPtr.Zero
                || // !IsTopMostWindow(fgWindow) ||
                fgWindow == deskWindow
                || fgWindow == shellWindow)
            {
                return false;
            }
            ...
            if (fgRect == screenRect)
            {
                ...
                if (classNameStr == "WorkerW"
                    ...
                    || classNameStr == "ImmersiveLauncher"
                    || //win8 开始屏幕
                ...
                Debug.WriteLine(
                    string.Format("Window[{0:x}] IsInFullScreenMode:", fgWindow.ToInt64()));
                return true;
            }
        ...
        protected void Dispose(bool disposing)
        {
            ...
            {
                Debug.WriteLine(
                    "Win32MousePathTracker2.Dispose(false) called by finalizer, which is probably dangerous.");
            }
