...
namespace WGestures.App.Migrate
{
    internal static class MigrateService
    {
        ...
        {
            var possibleDirs = Directory.GetDirectories(
                Directory.GetParent(AppSettings.UserDataDirectory).FullName);
            possibleDirs = possibleDirs.Where(
                    s =>
                    {
                        var dirName = Path.GetFileName(s);
                        int num;
                        return dirName.Split('.').Length == 4
                               && int.TryParse(dirName.Replace(".", string.Empty), out num);
                    })
                .ToArray();
            ...
            Func<string, int[]> splitToInts = s
                => Path.GetFileName(s).Split('.').Select(i => int.Parse(i)).ToArray();
        ...
        public static ConfigAndGestures Import(string from)
        {
            ...
            if (from.EndsWith(".json") || from.EndsWith(".JSON"))
            {
                return ImportJsonGestures(from, "1");
            }

            if (from.EndsWith(".wg") || from.EndsWith(".WG"))
            {
                return ImportJsonGestures(from, "2");
            }

            if (from.EndsWith(".wg2"))
            {
                return ImportJsonGestures(from, "2");
            }
        ...
        public static ConfigAndGestures Combine(
            ConfigAndGestures old, ConfigAndGestures current, MergeOption mergeOption)
        {
            var config = null as PlistConfig;
            var gestures = null as JsonGestureIntentStore;
        ...
        public static ConfigAndGestures ImportWgb(string wgbFilePath)
        {
            ...
            try
            {
                ...
                foreach (var fileName in files)
                {
                    ...
                    {
                        config = new PlistConfig(file.GetStream(), true);
                    }
                    else
                    {
