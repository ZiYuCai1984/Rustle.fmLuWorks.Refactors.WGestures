...
namespace WGestures.App.Gui
{
        ...
        where TBaseViewType : ICommandView
    {
        ...
        private readonly Dictionary<Type, TBaseViewType> _viewInstanceCache =
            new Dictionary<Type, TBaseViewType>();
        ...
        public TBaseViewType GetCommandView(AbstractCommand command)
        {
            ...
            foreach (var kv in _commandViewRegistry)
            {
                ...
                if (kv.Key == cmdType)
                {
                    return this.ConstructCommandView(kv.Value, command);
                }
            ...
            return closestParentKv.Key == null
                ? default
                : this.ConstructCommandView(closestParentKv.Value, command);
        }

        public TBaseViewType GetCommandView(Type commandType)
        {
            var cmd = Activator.CreateInstance(commandType) as AbstractCommand;
            return this.GetCommandView(cmd);
        }
            ...
            where TV : TBaseViewType
        {
            ...
            if (registeredType != null)
            {
                throw new InvalidOperationException(
                    string.Format("命令类型 {0} 已经注册到{1}", cmdType, registeredType));
            }
