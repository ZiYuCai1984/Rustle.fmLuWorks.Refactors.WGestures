...
namespace WGestures.App.Gui.Windows
{
    public partial class EditAppForm : Form
    {
            ...
            : this(intentStore)
        {
            ...
            this.Text = "修改 \"" + editingApp.Name + "\"";
            this.AppName = txtSelectedAppNae.Text = editingApp.Name;
            this.AppPath = lnkSelectedAppPath.Text = editingApp.ExecutablePath;
            ...
            this.LoadIcon(editingApp.ExecutablePath);

            if (!editingApp.AppExists())
            {
                this.ShowErrorMsg("该应用程序不存在，请确定它的位置");
            }
        }
        ...
        private void btnSelectFile_Click(object sender, EventArgs e)
        {
            ...
            {
                this.HandleSelectedApp(
                    dlgSelectFile.FileName,
                    Path.GetFileNameWithoutExtension(dlgSelectFile.FileName));
            }
            else
            {
        ...
        private void HandleSelectedApp(string path, string appName)
        {
            ...
            ExeApp found;
            var exist = _intentStore.TryGetExeApp(path, out found);
            ...
            {
                this.ShowErrorMsg("应用程序已经在列表中，请重新选择");
            }
        ...
        {
            pictureSelectedAppIcon.Image =
                IconHelper.ExtractIconForPath(path, new Size(48, 48), true, 0);
        }
        ...
        private void HackDragDrop()
        {
            ...
            ElevatedDragDropManager.Instance.ElevatedDragDrop +=
                this.panelDropFile_ElevatedDragDropHandler;
        }

        private void btnOk_Click(object sender, EventArgs e)
        {
            this.AppName = txtSelectedAppNae.Text;
        ...
        private void panelFileDrop_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (Path.GetExtension(path).Equals(".exe", StringComparison.CurrentCultureIgnoreCase)
                || Path.GetExtension(path)
                    .Equals(".msc", StringComparison.CurrentCultureIgnoreCase))
            {
                resultPath = path;
            }
            else if (Path.GetExtension(path)
                .Equals(".lnk", StringComparison.CurrentCultureIgnoreCase))
            {
                string target;
            ...
            this.HandleSelectedApp(resultPath, Path.GetFileNameWithoutExtension(path));
        }
        ...
        private void pictureWindowSelector_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            pictureWindowSelector.Image = Resources.block;
            this.Cursor = Cursors.Cross;

            var picBounds = pictureWindowSelector.Bounds;
            var cursorPos = pictureWindowSelector.PointToScreen(
                new Point(picBounds.Size.Width / 2, picBounds.Size.Height / 2));
        ...
        private void pictureWindowSelector_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            pictureWindowSelector.Image = Resources.cross;
            ...
            {
                this.HandleSelectedApp(
                    _mousePicedProcessFilePath,
                    Path.GetFileNameWithoutExtension(_mousePicedProcessFilePath));
            }
        }
        ...
        private static string GetLnkTarget(string lnkPath)
        {
            ...
            try
            {
                shl = new Shell(); // Move this to class scope
                lnkPath = Path.GetFullPath(lnkPath);
                dir = shl.NameSpace(Path.GetDirectoryName(lnkPath));
                itm = dir.Items().Item(Path.GetFileName(lnkPath));


                lnk = (ShellLinkObject) itm.GetLink;
            ...
            try
            {
                ...
                using (var fileReader = new BinaryReader(fileStream))
                {
                    ...
                    if ((flags & 1) == 1)
                    {
                        ...
                        uint offset =
                            fileReader.ReadUInt16(); // Read the length of the Shell item ID list
                        fileStream.Seek(
                            offset,
                            SeekOrigin.Current); // Seek past it (to the file locator info)
                    }

                    var fileInfoStartsAt =
                        fileStream.Position; // Store the offset where the file info
                    // structure begins
                    var totalStructLength =
                        fileReader.ReadUInt32(); // read the length of the whole struct
                    fileStream.Seek(0xc, SeekOrigin.Current); // seek to offset to base pathname
                    var fileOffset = fileReader.ReadUInt32(); // read offset to base pathname
                    // the offset is from the beginning of the file info struct (fileInfoStartsAt)
                    fileStream.Seek(
                        fileInfoStartsAt + fileOffset,
                        SeekOrigin.Begin); // Seek to beginning of
                    // base pathname (target)
                    var pathLength =
                        totalStructLength + fileInfoStartsAt - fileStream.Position - 2; // read
                    // the base pathname. I don't need the 2 terminating nulls.
                    var linkTarget =
                        fileReader.ReadChars((int) pathLength); // should be unicode safe
        ...
        {
            IWshShell wsh = null;
            IWshShortcut sc = null;

            try
            {
                wsh = new WshShell();
                sc = (IWshShortcut) wsh.CreateShortcut(lnkPath);
